...
class QCCuttingVerification(get_subid_model()):
    ...
    forecast = models.OneToOneField(
        Forecast, on_delete=models.CASCADE, related_name="qc_cutting_verifications"
    )
    ...
    class Meta:
        default_permissions = ()
        permissions = [
            ("view_cutting", "Can view qc cutting verification"),
            ("verify_cutting", "Can verify qc cutting verification"),
    ...
    def __str__(self):
        ...
        return f"QC Cutting for forecast {self.forecast.id}: {status}"
